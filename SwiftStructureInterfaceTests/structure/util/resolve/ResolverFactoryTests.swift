...
class ResolverFactoryTests: XCTestCase {
    ...
    func test_shouldNotCacheGenericArgumentInCaseItHasTheSameNameAsAnotherType() throws {
        let filePath1 = writeFile("func a<T>(a: T) {}\nfunc b(b: T){}")
        let filePath2 = writeFile("protocol T {}")
        let parsedFile = try ElementParser.parseFile(at: filePath1)
    ...
    func test_shouldNotCacheTypealiasArgumentInCaseItHasTheSameNameAsAnotherType() throws {
        let filePath1 = writeFile("protocol A { typealias T = A\nfunc a(a: T) }")
        let filePath2 = writeFile("func b(b: T)")
        let filePath3 = writeFile("protocol T {}")
        let parsedProtocol = try ElementParser.parseFile(at: filePath1).typeDeclarations[0]
        resolver = ResolverFactory.createResolver(filePaths: [filePath1, filePath2, filePath3])
        XCTAssert(resolveParameterType(parsedProtocol.codeBlock, at: 0) is TypealiasDeclaration)
        let parsedFile = try ElementParser.parseFile(at: filePath2)

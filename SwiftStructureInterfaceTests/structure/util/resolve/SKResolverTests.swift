...
class SKResolverTests: XCTestCase {
    ...
    func test_resolve_shouldResolveElementInSameFile() throws {
        resolver = createSameFileResolver()
        writeResolveClassesToSingleFile()
        let file = try ParserTestHelper.parseFile(fromPath: resolveFile)
    ...
    func test_resolve_shouldResolveSwiftTypeDeclaration() throws {
    ...
    func test_resolve_shouldResolveElementWithUTF16Characters() throws {
        resolver = createManyFileResolver()
    ...
    func test_resolve_shouldReturnNil_whenNoCursorInfo() throws {
    ...
    func test_resolve_shouldCallNotCallDecoratorWhenResolves() throws {
        resolver = createSpyingResolver()
        writeResolveClassesToSingleFile()
        let file = try ParserTestHelper.parseFile(fromPath: resolveFile)
        let reference = file.typeDeclarations[1].typeInheritanceClause.inheritedTypes[0]
